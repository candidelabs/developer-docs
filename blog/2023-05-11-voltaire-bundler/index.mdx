...
The [docker image](/wallet/bundler/installation#docker) is a single command line to get started running an instance of Voltaire, locally or on a private server. A bundler needs access to a **full node**. Run your own, or get one from a node provider.
...
A bundler needs to perform and handle multiple UserOperations requests at a time. Asyncio is ideal for handling multiple concurrent tasks efficiently. This enables the server to handle multiple client requests simultaneously without blocking other requests.

A big part of our inspiration for Voltaire is [Trinity](https://trinity.ethereum.org/), a previous open-source Ethereum client implementation written in Python. We would like to thank the Trinity team, as well as all Python community members for their open-source contributions.
