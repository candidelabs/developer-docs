...
The array is an ordered sequence. Entry `n` constrains the `n`th call of the UserOperation, and the UserOperation is sponsored only when it contains exactly this many calls, each matching its entry. Constraints within an entry must all pass for the call to qualify.
