...
```json
curl -X POST \
  https://api.candide.dev/recoveries/$version/$network/$your_api_key \
...
```json
curl -X GET \
  https://api.candide.dev/recoveries/$version/$network/$your_api_key/123456 \
  -H 'Content-Type: application/json'
```
...
```json
curl -X POST \
  https://api.candide.dev/recoveries/$version/$network/$your_api_key/execute/123456 \
  -H 'Content-Type: application/json' \
```
...
<TabItem value="request" label="Request Body">
    <DataTable items={postAlertSubscribe} / >
</TabItem>
<TabItem value="response" label="Response">
    <DataTable items={postAlertSubscribeResponse} / >
</TabItem>
</Tabs>
