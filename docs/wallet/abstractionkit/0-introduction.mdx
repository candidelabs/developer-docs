...
description: AbstractionKit is a typescript library to easily build and send UserOperations, with first class support for Safe Accounts, Calibur Accounts, and Simple EIP-7702 Accounts
...
| EIP-7702 account with passkeys and multi-key | `Calibur7702Account` | v0.8 | The Calibur account by Uniswap. Adds passkey authentication, multi-key management, and per-key hooks to EIP-7702 EOA upgrades. See [Calibur Account](/wallet/abstractionkit/calibur-account/). |
...
For EIP-7702 EOA upgrades, AbstractionKit supports [Calibur Account](/wallet/abstractionkit/calibur-account/) (by Uniswap, audited by OpenZeppelin and Cantina) with passkey authentication, multi-key management, and per-key hooks, as well as [Simple7702Account](/wallet/abstractionkit/simple-7702-account/) for lightweight single-signer upgrades.
...
The docs track AbstractionKit v0.4.0. Source links throughout the reference pages point at the `v0.4.0` tag. To install the documented version explicitly:

```bash
npm install abstractionkit@0.4.0
```
