...
import { userOperationParamV09 } from "/src/data/userOperation";
...
## Compatibility with Safe Account V3

`SafeMultiChainSigAccountV1` is API-compatible with [`SafeAccountV0_3_0`](/wallet/abstractionkit/safe-account-v3). Both extend the same `SafeAccount` base and expose the same methods with the same signatures, so moving to EntryPoint v0.9 is a change of account class — not a separate integration:
...
<TabItem value="Param type" label="Param Types">
    ...
    <tbody>
      ...
      <tr>
        ...
        <td align="left">
          CreateUserOperationV9Overrides, optional object
          <DataTable items={createUserOperationV7Overrides} />
        </td>
        <td align="left">
          CreateUserOperationV9Overrides allows you to override any of the default parameters for the user operation. It has the same fields as CreateUserOperationV7Overrides.
        </td>
      </tr>
    </tbody>
  </table>
  ...
  <table>
    ...
    <tbody>
      <tr>
        <td>UserOperation</td>
        <td align="left">
          <div>{`Promise<UserOperationV9>`}</div>
          <DataTable items={userOperationParamV09} />
        </td>
        <td align="left">
          Returns an unsigned UserOperation for EntryPoint v0.9.
        </td>
      </tr>
    </tbody>
  </table>
...
<TabItem value="Param type" label="Param Types">
  <table>
    ...
    <tbody>
      <tr>
        <td>userOperationsToSign</td>
        <td align="left">
          UserOperationToSign[]
          <DataTable items={userOperationToSignType} />
        </td>
        <td align="left">
          UserOperations with their chain IDs and optional validity windows.
        </td>
      </tr>
      <tr>
        <td>signers</td>
        ...
        <td align="left">
          One signer per owner required by the threshold, in any order. The built-in adapters `fromPrivateKey`, `fromViem`, `fromEthersWallet`, and `fromViemWalletClient` all work here.
        </td>
      </tr>
    </tbody>
  </table>
  ...
  <table>
    ...
    <tbody>
      <tr>
        ...
        <td align="left">
          One formatted signature per input UserOperation, in the same order.
        </td>
      </tr>
    </tbody>
  </table>
