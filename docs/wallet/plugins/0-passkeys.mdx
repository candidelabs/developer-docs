...
Install the Candide skills so your AI agent knows how to wire up Safe Passkeys (RP ID setup, public-key extraction, signature assembly, recovery posture). Source: [github.com/candidelabs/skills](https://github.com/candidelabs/skills).
...
Then ask: *"Integrate Safe Passkeys."*
...
Then ask Codex: *"Integrate Safe Passkeys."*
...
```
https://raw.githubusercontent.com/candidelabs/skills/main/skills/safe-passkeys/SKILL.md
```
...
AbstractionKit `v0.3.5` includes bigint-safe JSON helpers for storing and rehydrating passkey public coordinates:

```ts title="storePasskey.ts"
import {
    pubkeyCoordinatesFromJson,
    pubkeyCoordinatesToJson,
} from "abstractionkit";

localStorage.setItem("passkeyPubkey", pubkeyCoordinatesToJson(webauthPublicKey));

const stored = localStorage.getItem("passkeyPubkey");
const restoredPubkey = stored ? pubkeyCoordinatesFromJson(stored) : undefined;
```
...
Create a UserOperation following the standard Safe flow with [createUserOperation](/wallet/abstractionkit/safe-account-v3/#createuseroperation). Pass `expectedSigners` so that gas estimation accounts for the passkey signature format.
...
#### Sign with fromSafeWebauthn

AbstractionKit `v0.3.5` includes `fromSafeWebauthn`, an `ExternalSigner` adapter for Safe passkey owners. It handles the Safe-specific signer address, contract-signature encoding, and `signUserOperationWithSigners` integration.
...
The lower-level manual path is still available when you need direct control over each signature field. It involves four substeps: hashing, requesting a WebAuthn assertion, building the signature data, and formatting it.
