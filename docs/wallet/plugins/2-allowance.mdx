...
keywords: [allowance, safe, recurring, recurring-transfers, subscriptions]
...
The Safe Allowance Module enables users to automate token transfers securely. It provides a flexible framework allowing account owners to delegate specific rights and permissions to other accounts or contracts, ensuring funds can be transferred according to predefined conditions.

Unlike traditional ERC-20 token contract allowances, this module grants authorization for specific token amounts to delegates (accounts or smart contracts), enabling them to use tokens on behalf of users. It ensures limited access to specific tokens while facilitating automated transactions.

By leveraging the Safe Allowance Module, wallet developers can implement sophisticated financial operations—from managing delegated funds for multiple users and automating payment processes to delegating to smart contracts for various use cases.

## Use Cases
**Recurring Transfers**: Automate periodic transactions for recurring transfers. Users can define the amount, frequency, duration, and recipients.

**Subscriptions**: Set up automatic payments for subscriptions, utility bills, or membership fees.

**Dollar-Cost Averaging (DCA)**: Facilitate DCA strategies by delegating allowances to swap contracts.

**Custom Use Cases**: Delegate to smart contracts for application-specific needs.
...
1. Recommended methods offering simplified interfaces, documented under [Methods](/wallet/plugins/allowance/#methods).
2. Methods prefixed with `createBase` that match exact contract method names and parameters, documented under [Advanced Methods](/wallet/plugins/allowance/#advanced-methods).

While `createBase` methods enable direct contract interaction, they can be complex due to multiple options and parameters. The recommended methods improve usability by providing clearer, more intuitive method names and input parameters.


:::tip
If you're looking for a code example, take a look at the [Spend Permission example](https://github.com/candidelabs/abstractionkit-examples/blob/main/spend-permission/spend-permission.ts) and the [Allowance Module tests](https://github.com/candidelabs/abstractionkit/blob/main/test/safe/allowanceModule.test.js)  
:::
...
The example below demonstrates setting a recurring allowance using [createRecurringAllowanceMetaTransaction](/wallet/plugins/allowance/#createrecurringallowancemetatransaction). You can also set one-time allowances via [createOneTimeAllowanceMetaTransaction](/wallet/plugins/allowance/#createonetimeallowancemetatransaction).  
...
Batch all three MetaTransactions into a single UserOperation using [createUserOperation](/wallet/abstractionkit/safe-account-v3/#createuseroperation). Then proceed with the standard flow of signing and submitting the UserOperation on-chain.
...
### 6. Delegate Executes the Allowance

Once the allowance is set and conditions are met, the delegate can execute token transfers. Delegates can be automated services, EOAs (Externally Owned Accounts), or smart contracts.
...
:::info
Candide offers a secure and automated tool to execute delegated allowances. Contact us directly via our channels or email team@candidelabs.com.
:::
