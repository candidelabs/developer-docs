...
2. Those prefixed with `createBase`, which match the exact contract method names and parameters. These are under [Advanced Methods](/wallet/plugins/allowance/#advanced-methods).

While `createBase` methods allow direct interaction with the contract, they can be confusing for developers due to the multiple options and parameters involved. The recommended methods improves usability by providing clearer and more intuitive method names and input parameters.
