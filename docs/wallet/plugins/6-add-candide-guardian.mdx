---
title: Add Candide Guardian to Safe Account
description: Learn how to add Candide Guardian service to your Safe account for secure, user-friendly account recovery using email/SMS verification
...
Learn how to integrate [Candide Guardian](/wallet/recovery/auth-api/) as a trusted recovery service for users Safe accounts. Candide Guardian provides secure account recovery through email and SMS verification.
...
> You can also [fork the complete code](https://github.com/candidelabs/safe-recovery-service-sdk/blob/main/examples/03-candide-guardian-recovery/index.ts) and follow along.
...
```ts
// Create registration statement for SMS recovery
const siweStatementToSign = await recoveryService.createRegistrationToSmsRecoverySiweStatementToSign(
  smartAccount.accountAddress,
  "+1234567890"
);
