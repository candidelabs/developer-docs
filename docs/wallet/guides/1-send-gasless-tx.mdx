---
title: How to send a sponsored gas user operation using a paymaster with abstractionkit
...
If you are coming from the [previous guide](../getting-started), you only need one additional step. Skip directly to [step 5](./#step-5-get-paymaster-data).

<img src="/img/gasless-transaction-screen.png" style={{ width: "300px" }} />

## Quickstart
If you need, you can fork and run this tutorial directly on [Github](https://github.com/candidelabs/abstractionkit-examples/blob/main/sponsor-gas/sponsor-gas.ts)

### Prerequisites

- Node and a package manager (yarn or npm)
- Bundler API key from the [Candide Dashboard](https://dashboard.candide.dev) 
- An API key from the [dashboard](https://dashboard.candide.dev)
...
1. Create a new app on the [dashboard](https://dashboard.candide.dev), and copy the paymaster RPC URL in your .env 
2. Setup a Private Gas Policy on the dashboard, and copy its sponsor policy ID in your .env
3. Import `CandidePaymaster` and call the method [createsponsorpaymasteruseroperation](/wallet/abstractionkit/paymaster/#createsponsorpaymasteruseroperation)
