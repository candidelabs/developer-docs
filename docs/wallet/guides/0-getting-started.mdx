...
:::tip Already have users on EOAs?
This guide creates a new Safe account at a new address. To keep your users' existing addresses and add smart account features with EIP-7702, start with the [Calibur quickstart](/wallet/guides/getting-started-calibur/) instead.
:::
...
This guide uses the Safe Unified Account (`SafeMultiChainSigAccountV1`), which works for single-chain transactions and multichain flows with the same account class. See [Safe Account](/wallet/abstractionkit/safe-account/) for the account versions and how the `SafeAccount` alias maps to the real class.

#### Generate Account Address

```ts title="index.ts"
import * as dotenv from "dotenv";
// SafeMultiChainSigAccountV1 is the Safe Unified Account, aliased to SafeAccount for brevity.
