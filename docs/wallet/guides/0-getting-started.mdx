...
This guide uses the Safe Unified Account (`SafeMultiChainSigAccountV1`), which works for single-chain transactions and multichain flows with the same account class. See [Account Types](/wallet/abstractionkit/account-types/) for when to use each account class and how the `SafeAccount` alias maps to the real class.
