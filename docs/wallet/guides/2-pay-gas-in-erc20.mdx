---
title: How to send user operation with gas payed in erc-20 using a paymaster with abstractionkit
...
If you are coming from the [previous guide](../send-gasless-tx), you only need adjust the paymaster step. You will need an API key from the [dashboard](https://dashboard.candide.dev), and then you can skip directly to [step 5](./#step-5-get-paymaster-data).
...
- Bundler and Paymaster API key from the [Candide Dashboard](https://dashboard.candide.dev) 
- Some Candide Test Tokens (CTT). Request CTT on our [Discord](https://discord.gg/8q2H6BEJuf)
- If you need, fork the full example of this tutorial on on [Github](https://github.com/candidelabs/abstractionkit-examples/blob/main/pay-gas-in-erc20/pay-gas-in-erc20.ts)
...
```bash
mkdir candide-pay-gas-erc20
cd candide-pay-gas-erc20
npx tsc --init
```
...
1. Import `CandidePaymaster`, and initiate an instance with its URL. You can get a free API key on the [dashboard](https://dashboard.candide.dev).
