...
- The output token on the destination chain may have different decimals than the input token. Use `outputTokenDecimals` from [`forwarding_estimateOutput`](./forwarding-address-api.mdx#forwarding_estimateoutput) when formatting.
