...
The `custodialWithdrawer` is the emergency recovery address. If funds get stuck in a forwarding address (e.g. the relayer did not process them), both the `recipient` and the `custodialWithdrawer` can withdraw directly from the deployed contract on the source chain. The recipient can always withdraw immediately. The `custodialWithdrawer` can withdraw after a timelock.
...
|----------|-------------|------------------------|----------|
| Wallet or dapp (recommended) | User's address | Platform's address | Platform can recover stuck funds on behalf of the user after a timelock. User can always withdraw immediately |
